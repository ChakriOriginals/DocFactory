...
| 6 | **A corrupt drop is recorded, not lost** | `printf 'not a pdf' > /tmp/corrupt.pdf; aws s3 cp /tmp/corrupt.pdf s3://$BUCKET/dev-tenant/dropbox/invoice/corrupt.pdf` — then `GET /documents?status=failed` | a document with status `failed` and `last_error` naming the missing `%PDF-` header. **No DLQ message**, and that is correct: the file will not become a PDF on a third read, so retrying it three times to bury it in a DLQ is worse than recording it once. This row previously did not exist at all — the object was dropped with no trace anywhere. |
| 7 | **`needs_ocr` is terminal, never the DLQ** | drop a scanned/image-only PDF | status `needs_ocr`, DLQ depth unchanged. It should also appear in `GET /documents?status=needs_ocr` and in `/usage`'s `status_counts` — a terminal state that produces nothing still has to be countable. |
| 7b | **An uppercase extension still ingests** | `aws s3 cp /tmp/inv.pdf s3://$BUCKET/dev-tenant/dropbox/invoice/INV.PDF` | a document row appears. S3 suffix filters are case-sensitive, so before the second `.PDF` notification rule this generated no event at all — no message, no row, no log line, the object sitting in the bucket while the client believed it was sent. |
